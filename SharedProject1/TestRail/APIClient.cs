...
namespace SharedProject.TestRail
{
    public class APIClient
    {
        ...
        private object SendRequest(string method, string uri, object data)
        {
            ...
            for (int retry_num = 1; retry_num < 10 && result == null; retry_num++)
            {
                ...
                catch (WebException e)
                {
                    if (e.Response == null)
                    {
                        // commenting below to allow for HTTP retries if TestRail API fails

                        //throw;

                        result = null;
                        Log.WriteLine("TestRail API failed on attempt " + retry_num + " of 10.");
                    }
